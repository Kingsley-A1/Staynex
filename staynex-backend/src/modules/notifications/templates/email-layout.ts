...
export function bulletList(items: string[]): string {
  const rows = items
    ...
    .join("");
  return rows
    ? `<ul style="margin:0 0 18px;padding-left:22px;color:#101014;font-size:15px;line-height:1.55;">${rows}</ul>`
    : "";
}
...
export function renderEmailLayout(input: EmailLayoutInput): RenderedEmail {
  ...
  const detailRows = details
    .map(
      ({ label, value }) =>
        `<tr><td valign="top" style="padding:10px 8px 10px 0;border-bottom:1px solid #E7E5F2;color:#6E6A83;font-size:13px;line-height:1.4;">${escapeHtml(label)}</td><td valign="top" align="right" style="padding:10px 0 10px 8px;border-bottom:1px solid #E7E5F2;color:#101014;font-size:14px;font-weight:600;line-height:1.4;overflow-wrap:anywhere;">${escapeHtml(value)}</td></tr>`,
    )
    .join("");
  const detailTable = detailRows
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:20px 0;border-collapse:collapse;">${detailRows}</table>`
    : "";
  ...
  const support = input.support
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin-top:24px;background:#F7F7FF;border:1px solid #E7E5F2;"><tr><td style="padding:16px;"><p style="margin:0;color:#6E6A83;font-size:13px;line-height:1.55;">${escapeHtml(input.support)} <a href="mailto:support@staynexbookings.ng" style="color:#27187D;text-decoration:underline;">support@staynexbookings.ng</a></p></td></tr></table>`
    : "";
  const reason = input.reason
    ? `<p style="margin:22px 0 0;color:#6E6A83;font-size:12px;line-height:1.5;">${escapeHtml(input.reason)}</p>`
    : "";
  ...
  const text = [
    ...
    ...(input.support
      ? ["", `${input.support} support@staynexbookings.ng`]
      : []),
