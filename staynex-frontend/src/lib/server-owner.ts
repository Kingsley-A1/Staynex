// Server-only fetch helpers for the owner property-authoring surface. Auth is
// session-only: the request's cookies are forwarded to the backend, which
// resolves the owner. Properties are real platform state, so there is no fixture
// fallback for the owner's own listings. Cities fall back to display-only
...
export async function getOwnerProperties(): Promise<PropertySummary[] | null> {
  try {
...
export async function getOwnerProperty(id: string): Promise<PropertyDetail | null> {
  try {
